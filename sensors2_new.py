...
3. Localisation: Géolocalisation IP du device (ou forcée sur Casablanca)

Architecture:
- MQ135Sensor: Classe pour le capteur MQ-135
- DHT11Sensor: Classe pour le capteur DHT11
- GPSSensor: Classe pour la localisation (forcée sur Casablanca)
...
class GPSSensor:
    ...
    def read(self):
        """Return fixed GPS coordinates (Casablanca) from config.

        This implementation forces the device location to the configured
        map center (Casablanca) to ensure GPS is always within the city.
        """
        try:
